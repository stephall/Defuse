...
class DenoisingModel(torch.nn.Module):
    def __init__(self, x_dim, t_dim=1, lat_dim=100):
        ...
        self.linear_3 = torch.nn.Linear(self.lat_dim, self.x_dim)
...
class ConditionalDenoisingModel(torch.nn.Module):
    def __init__(self, x_dim, t_dim=1, y_dim=1, lat_dim=100):
        # Initialize the parent class
        super().__init__()
        ...
        # Define the input and output dimension, based on the
        self.input_dim  = x_dim + t_dim + y_dim
    ...
    def forward(self, data_dict, t):
        ...
        # print(f"y_encoded[:10]: {y_encoded[:10]}")

        # Concatenate x and the encoded t along the feature axis (second axis)
        xty = torch.cat([x, t_encoded, y_encoded], dim=1)
        # print(f"xty.shape: {xty.shape}")
        # print(f"xty[:10]: {xty[:10]}")
